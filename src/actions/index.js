...
//       error => dispatch(ApiFetchError(true, error))
//     )
//     .then(() => dispatch(apiHasLoaded(true)));
// };
...
const byYear = (exchange, fromSymb, toSymb) => dispatch =>
  axios
    .get(
      `https://min-api.cryptocompare.com/data/histoday?fsym=${fromSymb}&tsym=${toSymb}&limit=29&aggregate=1&e=${exchange}`
    )
    .then(
...
const topExchanges = (from, to) => dispatch =>
  axios
    .get(
